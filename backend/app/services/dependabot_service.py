"""
GitHub Dependabot alert sync service.

Polls the GitHub Dependabot Alerts API for each registered repository and
imports open alerts as Nyx findings.  Requires GITHUB_TOKEN with
`security_events` scope (or `repo` scope for private repos) and Dependabot
must be enabled in each repository's Security settings.
"""
...
logger = logging.getLogger("nyx.dependabot")
...
async def sync_repository(
    ...
    *,
        ...
        if last:
            ...
            if age < settings.CODE_SCANNING_POLL_INTERVAL:
                return {
                    ...
                    "next_sync_in": int(settings.CODE_SCANNING_POLL_INTERVAL - age),
                }
    ...
    if alerts is None:
        return {"status": "skipped", "reason": "Dependabot not enabled for this repository"}
...
async def sync_all_repositories() -> None:
    ...
    logger.info(f"Dependabot sync: checking {len(repos)} repositories")
    for repo in repos:
        try:
            result = await sync_repository(repo.id, repo.github_full_name)
            if result["status"] == "ok" and result.get("alerts_found", 0) > 0:
                logger.info(
                    f"Dependabot sync: {repo.github_full_name} — "
                    f"{result['alerts_found']} alerts imported"
                )
        except Exception as e:
            logger.warning(f"Dependabot sync failed for {repo.github_full_name}: {e}")
...
async def run_poll_loop() -> None:
    settings = get_settings()
    logger.info(
        f"Dependabot poll loop started (interval: {settings.CODE_SCANNING_POLL_INTERVAL}s)"
    )
    await asyncio.sleep(90)  # Stagger from code scanning poll start
    while True:
        try:
            await sync_all_repositories()
        except Exception as e:
            logger.error(f"Dependabot poll loop error: {e}")
    ...
    async with httpx.AsyncClient(base_url=_GITHUB_API, headers=headers, timeout=30.0) as client:
        while True:
            ...
            if resp.status_code == 404:
                return None  # Dependabot not enabled
            if resp.status_code == 403:
                logger.warning(
                    f"GitHub 403 for {owner}/{repo} Dependabot — "
                    "token may lack `security_events` scope or Dependabot is disabled"
                )
                return []
